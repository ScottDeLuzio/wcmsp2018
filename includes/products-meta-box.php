...
/* WordPress action 'add_meta_boxes' tells WordPress to run the function 'wcmsp2018_add_custom_box'
 ...
 * a function that will be run inside the meta box to output certain content (wcmsp2018_custom_box_html)
 *
 ...
 */
add_action( 'add_meta_boxes', 'wcmsp2018_add_custom_box' );

function wcmsp2018_add_custom_box(){
	$screens = array( 'sport-product' );
	foreach ( $screens as $screen ) {
		add_meta_box(
			'amazon_link',					// Unique ID
			'Product Link',  				// Box title
			'wcmsp2018_custom_box_html',	// Content callback, must be of type callable
			$screen							// Post type
		);
	}
}
 ...
 */
function wcmsp2018_custom_box_html( $post ){
	$value = get_post_meta( $post->ID, 'amazon_link', true ); 									// Retrieves the value of 'amazon_link' for this post
	?>
	<label for="amazon_link"><?php _e( 'Enter link for this product', 'wcmsp2018' ); ?></label> <!-- Outputs a label for the user to read -->
...
/* WordPress action 'save_post' runs whenever a post is saved or updated. Here we tell it to run the function 'wcmsp2018_save_postdata'.
 ...
 */
add_action( 'save_post', 'wcmsp2018_save_postdata' );

function wcmsp2018_save_postdata( $post_id ){
